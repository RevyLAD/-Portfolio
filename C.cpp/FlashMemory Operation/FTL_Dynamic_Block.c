	...
	for (i = 0; i < block; i++)
	{
		if (table[i].lbn == map_lbn) // lbn에 대응하는 pbn의 값을 매핑테이블에서 찾음
		{
			if (table[i].pbn == -1)
			{
				for (k = 0; k < block; k++)
				{
					if (strcmp(memory[k*SECTOR_SIZE + offset].memory, "-1") == 0)
					{
						Flash_write(memory, table, k*SECTOR_SIZE + offset, data);
						write_count++;
						write_Allcount++;
						table[i].pbn = k;
						break;
			...
			{
				if (!(strcmp(memory[(i * SECTOR_SIZE) + offset].memory, "-1") == 0)) //메모리에 데이터가 있으면
				{
				...
				else
				{
							...
							Flash_write(memory, table, i*SECTOR_SIZE + offset, data);
							write_count++;
							write_Allcount++;
							table[i].pbn = i;
							break;
